...
def install_for_batch(files, keep_source):
    """Installiert Grounding DINO Dateien (für Batch Mode)

    Args:
        files: Liste von file dicts aus scan_for_batch()
        keep_source: True (keep), False (delete)

    Returns:
        dict: {'installed': int, 'errors': int}
    """
    stats = {'installed': 0, 'errors': 0}

    for file_info in files:
        source_path = file_info['path']
        target_path = file_info['target_folder'] / file_info['new_name']

        success, final_path, msg = handle_duplicate_move(
            source_path, target_path, keep_source=keep_source
        )
