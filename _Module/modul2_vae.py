...
def install_for_batch(files_to_install, keep_source):
    """Install Regular VAE + VAE Approx files (called from batch processing)"""
    installed = 0
    errors = 0
    collisions = 0

    # Ensure both target folders exist
    ...
    for file_info in files_to_install:
        file_type = file_info.get('type', 'vae')  # Default to 'vae' for backwards compatibility
        result = file_info['result']

        # Get source path (different keys for VAE vs VAE Approx)
        ...
        # Use simplified handle_duplicate_move
